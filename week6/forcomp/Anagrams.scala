...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toList.filter(x => alphabets.contains(x)).
      groupBy(e => e).transform((x, y) => y.length).
      toList.sortBy((x: (Char, Int)) => x._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val occur_flat_list = occurrences.map(x => (for (i <- 1 to (x._2)) yield (x._1, i)).toList)
    occur_flat_list.foldRight(List[Occurrences](Nil))((x, y) => y ++ (for (i <- x; j <- y) yield (i :: j)))
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentence_anagram_loop(occurrences: Occurrences) : List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
        ...
        for {
          ...
          s <- sentence_anagram_loop(subtract(occurrences, i))
        } yield (j :: s)
      }
    }
    sentence_anagram_loop(sentenceOccurrences(sentence))
  }
